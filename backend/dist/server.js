...
// 发送验证码（仅邮箱）
const issueVerificationCode = async (email, purpose = 'login') => {
...
// 验证验证码
const verifyCode = async (email, code, purpose = 'login') => {
...
app.post('/api/photos/upload', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        const photo = await PhotoModel.create({
            ...
            tags: tags ? tags.split(/[,，]/).map((t) => t.trim()).filter(Boolean) : ['精选'],
        });
        res.json({
            id: photo._id.toString(),
            url: photo.url,
...
// 检查用户对某些照片的点赞状态
app.get('/api/photos/likes', async (req, res) => {
    const userId = req.query.userId;
    const photoIds = req.query.photoIds?.split(',').filter(Boolean);
    ...
    try {
        ...
        const likeMap = {};
        photoIds.forEach((id) => { likeMap[id] = false; });
        likes.forEach((like) => { likeMap[like.photoId] = true; });
...
app.get('/api/tags/popular', async (_req, res) => {
    try {
        ...
        res.json({
            tags: result.map((item) => ({ name: item._id, count: item.count })),
        });
    }
    catch (error) {
